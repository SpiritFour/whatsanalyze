import type { Message, EmojiAnalysis, AuthorEmojiData, EmojiStats } from '../types';

export class EmojiAnalyzer {
  private static emojiRegex = /\p{Extended_Pictographic}/gu;

  static getMostUsedEmojis(messages: Message[]): EmojiAnalysis {
    ...
    for (const message of messages) {
      ...
      const emojis = text.match(EmojiAnalyzer.emojiRegex);
      if (!emojis) continue;
    ...
    const authors: Record<string, { top5Emojis: EmojiStats[]; messageWithMostEmojis: Message | null }> = {};
